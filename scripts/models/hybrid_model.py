...
class HybridModel:
    model: "Mass_Conserving_ODE"

    def __init__(self, filepath, mask: List[int], model_config: dict = None):
        ...
        self.model = Mass_Conserving_ODE(self.S, 
                                         self.mask, 
                                         state_dim=self.n_metabolites, 
    ...
    def loss_fn(model, ts, batch_y0, batch_params, batch_ys, key, weight_decay):
        ...
        def rollout(y0, mech_params, k):
            return model(ts, y0, mech_params, key=k)
    ...
    def train_step(model, opt_state, ts, batch_y0, batch_params, batch_ys, optimizer, key, weight_decay):
        ...
        loss, grads = HybridModel.loss_fn(model, ts, batch_y0, batch_params, batch_ys, key, weight_decay)
    ...
    def train(self, ts, ys, params, val_ys=None, val_params=None, training_config: dict = None):
        ...
        min_delta = training_config.get("min_delta", 1e-10)
        ...
        first_step_timeout = training_config.get("first_step_timeout", 1200)
        lr_warmup_steps = training_config.get("lr_warmup_steps", 100)
        nan_divergence_threshold = training_config.get("nan_divergence_threshold", 100) # consecutive steps which are NaN/Diverged
        ...
        if self.normalized:
            ...
            # model pytree by swapping the inner HybridODE for a new one that
            # carries the data-derived normalisation constants as static values.
            old_func = self.model.func
            new_func = HybridODE.__new__(HybridODE)
            object.__setattr__(new_func, "mech_flux",      old_func.mech_flux)
            object.__setattr__(new_func, "nn_flux",        old_func.nn_flux)
            object.__setattr__(new_func, "S",              old_func.S)
            object.__setattr__(new_func, "mask",           old_func.mask)
            object.__setattr__(new_func, "substrate_mask", old_func.substrate_mask)
            object.__setattr__(new_func, "y_min",    np.array(y_min))
            object.__setattr__(new_func, "y_range",  np.array(y_range))
            object.__setattr__(new_func, "p_min",    np.array(p_min))
            object.__setattr__(new_func, "p_range",  np.array(p_range))
        ...
        if resume and checkpoint_path and Path(checkpoint_path).exists():
            try:
                ...
                if self.normalized and "y_min" in metadata and metadata["y_min"] is not None:
                    ...
                    object.__setattr__(new_func, "mask",           old_func.mask)
                    object.__setattr__(new_func, "substrate_mask", old_func.substrate_mask)
                    object.__setattr__(new_func, "y_min",    np.array(metadata["y_min"]))
                    object.__setattr__(new_func, "y_range",  np.array(metadata["y_range"]))
                    object.__setattr__(new_func, "p_min",    np.array(metadata["p_min"]))
                    object.__setattr__(new_func, "p_range",  np.array(metadata["p_range"]))
                    object.__setattr__(new_func, "normalized",     True)
                    object.__setattr__(new_func, "nn_scale",       old_func.nn_scale)
                    self.model = eqx.tree_at(lambda m: m.func, self.model, new_func)
        ...
        for step in range(start_step, max_steps):
            ...
            try:
                ...
                # First-step check to avoid spending time on models that already diverged at the first step
                if step == start_step:
                ...
                if n_val > 0:
                    # Only check validation loss after 500 steps to avoid early stopping due to initial instability
    ...
    def save(self, path: str):
        ...
        metadata = {
            ...
            "atol": self.atol,
            "seed": self.seed,
            "y_min": self.model.func.y_min.tolist() if self.normalized else None,
            "y_range": self.model.func.y_range.tolist() if self.normalized else None,
            "p_min": self.model.func.p_min.tolist() if self.normalized else None,
            "p_range": self.model.func.p_range.tolist() if self.normalized else None
        }
        import json
    ...
    def save_checkpoint(self, path: str, step: int, opt_state, best_val_loss: float, best_train_loss: float):
        ...
        metadata = {
            ...
            "best_loss": float(best_val_loss), # for backward compatibility
            "filepath": str(self.filepath),
            "mask": self.mask.tolist(),
            "width_NODE": self.width_NODE,
            "depth_NODE": self.depth_NODE,
    ...
    def load(cls, path: str):
        ...
        config_dict = run_meta.get("config", {})
        
        width = metadata.get("width_NODE", config_dict.get("width_NODE", run_meta.get("width", 32)))
        depth = metadata.get("depth_NODE", config_dict.get("depth_NODE", run_meta.get("depth", 2)))
        normalize = metadata.get("normalize", config_dict.get("normalize", run_meta.get("normalize", True)))
        dropout = metadata.get("dropout", config_dict.get("dropout", run_meta.get("dropout", 0.1)))
        solver = metadata.get("solver", config_dict.get("solver", run_meta.get("solver", "Kvaerno5")))
        dt0 = metadata.get("dt0", config_dict.get("dt0", run_meta.get("dt0", 1e-4)))
        rtol = metadata.get("rtol", config_dict.get("rtol", run_meta.get("rtol", 1e-4)))
        atol = metadata.get("atol", config_dict.get("atol", run_meta.get("atol", 1e-6)))
        seed = metadata.get("seed", config_dict.get("seed", 0))
...
class KineticModel(eqx.Module):
    funcs: dict = eqx.static_field()
    argument_names: dict = eqx.static_field()
    reaction_names: tuple = eqx.static_field()
    species_names: tuple = eqx.static_field()
    param_index: dict = eqx.static_field()

    def __init__(self, kinetic_model, param_index):
        self.funcs = kinetic_model.func.func
        self.argument_names = kinetic_model.func.argument_names
        self.reaction_names = tuple(kinetic_model.reaction_names)
        self.species_names = tuple(kinetic_model.species_names)
        self.param_index = param_index

    def __call__(self, t, y, mech_params):

        y_dict = dict(zip(self.species_names, y))
        fluxes = []

        for rxn in self.reaction_names:
            func = self.funcs[rxn]
            argnames = self.argument_names[rxn]
            ...
            for name in argnames:
                if name == "t":
                    ...
                    args[name] = y_dict[name]
                else:
                    args[name] = mech_params[self.param_index[name]]

            fluxes.append(func(**args))
...
class Neural_ODE(eqx.Module):
    ...
    def __init__(self, state_dim, param_dim, n_reactions, width, depth, key, dropout):
        ...
        layers.append(eqx.nn.Linear(width, n_reactions, key=keys[-1]))
        
        # Near zero weight initilization
...
class HybridODE(eqx.Module):
    mech_flux: KineticModel
    nn_flux: Neural_ODE
    S: jnp.ndarray = eqx.field(static=True)
    mask: jnp.ndarray = eqx.field(static=True)
    substrate_mask: jnp.ndarray = eqx.field(static=True)
    ...
    def __init__(self, S, mask, state_dim, param_dim, n_reactions, width, depth, key, kinetic_model, param_index, dropout, normalized):
        self.nn_flux = Neural_ODE(state_dim, param_dim, n_reactions, width, depth, key, dropout)
        self.mech_flux = KineticModel(kinetic_model, param_index)
        self.S = S
        self.mask = mask
        self.normalized = normalized

        self.substrate_mask = (S < 0).astype(jnp.float32)

        self.y_min = np.zeros(state_dim)
        self.y_range = np.ones(state_dim)
        self.p_min = np.zeros(param_dim)
        self.p_range = np.ones(param_dim)
    ...
    def __call__(self, t, y, args):
        mech_params, key = args
        
        if self.normalized:
            y_min   = jnp.asarray(self.y_min)
            y_range = jnp.asarray(self.y_range)
            p_min   = jnp.asarray(self.p_min)
            p_range = jnp.asarray(self.p_range)

            # Recover physical values for Mechanistic Model
            y_phys = y * y_range + y_min
            p_phys = mech_params * p_range + p_min
            
            # Mechanistic part needs actual values
            y_phys = jnn.relu(y_phys)

            # For NN use normalized values
            y_safe = jnn.relu(y)
            v_nn = self.nn_scale * jnp.clip(self.nn_flux(y_safe, mech_params, key=key), -50.0, 50.0)

            k = 50.0  # sharpness
            is_positive = jnn.sigmoid(k * y_phys)[:, None]
            gates = jnp.where(self.substrate_mask, is_positive, 1.0)
            substrate_gate = jnp.prod(gates, axis=0)

            v_nn = v_nn * substrate_gate

            v_phi = self.mech_flux(t, y_phys, p_phys)

            v_mech_masked = v_phi * (1.0 - self.mask)

            v_total_phys = v_nn + v_mech_masked
            dydt_phys = self.S @ v_total_phys

            dydt_norm = dydt_phys / y_range
            dydt_norm = jnp.clip(dydt_norm, -50.0, 50.0)

            return dydt_norm

        else:
            v_nn = 1e-3 * jnp.clip(self.nn_flux(y, mech_params, key=key), 0.0, 1e2)
            
            eps = 1e-4
            y_gate = jnp.maximum(y, 0.0)
            substrate_concs = y_gate[:, None] * self.substrate_mask
            gates = substrate_concs / (substrate_concs + eps)
            gates = jnp.where(self.substrate_mask, gates, 1.0)
            substrate_gate = jnp.prod(gates, axis=0)

            v_nn = v_nn * substrate_gate
            
            v_phi = self.mech_flux(t, y, mech_params)
    
            v_mech_masked = v_phi * (1.0 - self.mask)

            v_total = v_nn + v_mech_masked
...
class Mass_Conserving_ODE(eqx.Module):
    func: HybridODE
    solver_name: str
    dt0: float
    rtol: float
    ...
    def __init__(self, S, mask, state_dim, param_dim, n_reactions, width, depth, key, kinetic_model, param_index, dropout, normalized, solver, dt0, rtol, atol):
        self.func = HybridODE(S, mask, state_dim, param_dim, n_reactions, width, depth, key, kinetic_model, param_index, dropout, normalized)
        self.solver_name = solver
        self.dt0 = dt0
        self.rtol = rtol
        self.atol = atol

    def __call__(self, ts, y0, mech_params, key=None):

        def event_fn(t, y, args, **kwargs):
         
            if self.func.normalized:
                return jnp.any(y < -10) | jnp.any(y > 1e7)
                # return jnp.any(y < -100)
            else:
             
                return jnp.any(y < -10) | jnp.any(y > 1e7)
        
        term = diffrax.ODETerm(
            eqx.Partial(self.func)
        )
