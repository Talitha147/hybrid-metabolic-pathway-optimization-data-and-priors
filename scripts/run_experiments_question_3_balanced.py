...
def main():
    ...
    tracker_path = "Experiments/Question_3_balanced/experiment_tracker.csv"
    ...
    def run(model_path, name, M = None):
        ...
        for ts_indices in indices_list:
            for strain_num in strain_nums:
                base_path="Experiments/Question_3_balanced/"
    ...
    model_paths = [model_dir + "/pCA_model_lumped_1_part_1_balanced.xml", model_dir + "/pCA_model_lumped_2_part_1_balanced.xml", model_dir + "/pCA_model_lumped_1_part_1_sink_balanced.xml", model_dir + "/pCA_model_lumped_2_part_1_sink_balanced.xml"]
    model_names = ["lumped_1_part_1_balanced", "lumped_2_part_1_balanced", "lumped_1_part_1_sink_balanced", "lumped_2_part_1_sink_balanced"]
    
    for model_path, name in zip(model_paths, model_names):
        run(model_path, name)

    model_paths = [model_dir + "/pCA_model_lumped_1_part_2_balanced.xml", model_dir + "/pCA_model_lumped_1_part_2_sink_balanced.xml"]
    model_names = ["lumped_1_part_2_balanced", "lumped_1_part_2_sink_balanced"]

    # Lumped reactions are masked, others are considered known. Sinks masked.
    masks = [jnp.array([1]), jnp.array([1, 1])]
